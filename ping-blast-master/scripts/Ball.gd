...
func _ready():
	...
	if is_in_group("ball"):
		connect("spawn_small", Callable(get_node(".."), "_on_Ball_spawn_small"))
	else:
	...
	connect("add_score", Callable(get_node("../CanvasLayer/HUD"), "_on_Ball_add_score"))
	connect("explode", Callable(get_node(".."), "_on_Ball_explode"))
	...
	if (percent > 0.8) and not is_in_group("ball_small"):
		...
		strengh = 10
		is_bonus = true
		connect("bonus_get", Callable(get_node("../Player"), "bonus"))
