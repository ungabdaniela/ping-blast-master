...
func _physics_process(delta):
	if Input.is_action_pressed("click") and can_shoot: #Quand le clic gauche est appuyé
		var bullet = bullet_ressource.instantiate() #Instance en tant que noeud
		add_child(bullet) #L'ajoute dans la scène
		if multi_bullet_enabled:
			var multi_bullet = multi_bullet_ressource.instantiate() #Instance en tant que noeud
...
func toast():
	var ball_small = ball_small_ressource.instantiate() #Instance en tant que noeud
	ball_number += 1
...
func _on_SpawnNewTimer_timeout():
	var ball = ball_ressource.instantiate() #Instance en tant que noeud
	ball_number += 1
